...
The diagram also details where a number of popular cryptocurrencies fit into the consensus model. Post merge, ETH 2 for example uses a hybrid approach when it comes to the consensus model. In addition, coins like Cardano use longest chain while also using proof of stake sybil resistance (see below). At a high level, 
